...
Some code chunks are hidden by default (mostly for plots), but you can easily unfold them, by clicking on the "code" tab.

### Task

Transform variables according to guidelines, describe dataset and perform EDA. Develop a Bayesian regression model to predict `audience_score` from the selected variables. Make a prediction for a previously unseen data (cf. [project requirements](requirements/info4_bayesian)).
...
design %>% tibble::rownames_to_column() %>% 
  ...
  select( rowname | which(colSums(is.na(.)) > 0) ) %>% 
  knitr::kable()
```
\
...
$$ \widehat{\texttt{audience_score}} =  62.3 - 17.9 \times (\texttt{feature_film}_\mathrm{yes}^\phantom{1} - 0.91) + 3.8 \times (\texttt{drama}_\mathrm{yes}^\phantom{1} - 0.47) + \\  0.42 \times (\texttt{critics_score} - 57.6) - 0.17 \times (\texttt{thtr_rel_year} - 1998) + 7.2 \times \log_{10} \left(\frac{\texttt{imdb_num_votes}}{16568}\right)$$
