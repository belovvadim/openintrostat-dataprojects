...
The code chunks are hidden by default, for easier reading. You can freely unfold them at will, by clicking on "code" tab.
...
#library(printr)     # nicer tabular output
#options(knitr.table.NA = "")
```

### Task

Describe movies dataset and perform EDA. Develop a multiple linear regression model to predict a numerical variable, that describes movie's popularity. Do a prediction for previously unseen data (cf. [project requirements](requirements/info3_reg_model.md)).
...
```{r}
knitr::kable(design[!complete.cases(design), colSums(is.na(design)) > 0])
```

There can be no movie without runtime:
```{r}
filter(movies, is.na(runtime)) %>% knitr::kable() %>% 
  kableExtra::kable_styling("striped") %>% kableExtra::scroll_box(width = "100%")
```
...
filter(movies, best_pic_win == "yes" & best_pic_nom == "no") %>%
  knitr::kable() %>% kableExtra::kable_styling("striped") %>% kableExtra::scroll_box(width = "100%")
```
...
knitr::kable(as.data.frame(X_nom)) %>%
  kableExtra::kable_styling("striped") %>% kableExtra::scroll_box(height = "400px", width = "100%")
```
...
$$\widehat{\texttt{imdb_norm}} = \beta_0 + \pmb{\beta}_1^T \pmb{I}_{\texttt{genre}}^{\scriptsize\text{ref: Documentary}} + \pmb{\beta}_5^T \pmb{I}_{\texttt{mpaa_rating}}^{\scriptsize\text{ref: G}} +\beta_6^\phantom{T} I_{\texttt{title_type}}^{\scriptsize\text{Feature or TV}} + \beta_{11}^\phantom{T} I_{\texttt{awards_pic}}^{\scriptsize\text{Yes}} + \beta_{12}^\phantom{T} I_{\texttt{dvd_lag}}^{\scriptsize\text{Yes}} +  \\
(\beta_4^\phantom{T} + \pmb{\beta}_7^T \pmb{I}_{\texttt{genre}}^{\scriptsize\text{ref: Documentary}} ) \times \texttt{thtr_rel_year} + (\beta_9^\phantom{T} + \pmb{\beta}_{10}^T \pmb{I}_{\texttt{genre}}^{\scriptsize\text{ref: Documentary}} ) \times \texttt{quickness} + \\ 
(\beta_3^\phantom{T} + \pmb{\beta}_8^T \pmb{I}_{\texttt{genre}}^{\scriptsize\text{ref: Documentary}} + \beta_{13}^\phantom{T} I_{\texttt{dvd_lag}}^{\scriptsize\text{Yes}}) \times \texttt{log_votes} + \beta_2^\phantom{T} \times (\texttt{log_votes} - 11.33)_+ $$

Here the notation like $I_{\texttt{title_type}}^{\scriptsize\text{Feature or TV}}$ denotes indicator variable, taking the value 1 for specified level of a binary predictor. The bold font denotes the vector of such indicator variables, with respect to specified reference level. The interpretation of the first line, thus, is that it gives average IMDB score `imdb_norm`, whose value w.r.t. baseline $\beta_0$ changes for each category by the corresponding coefficients $\beta_i$, provided below:
```{r}
knitr::kable(model_nom$coefficients) %>% 
  kableExtra::kable_styling("striped") %>% kableExtra::scroll_box(height = "400px")
```

Recall that the predicted score is related to the original IMDB rating [1,10] as follows:
$$\texttt{imdb_rating} = 10 * \left( 1 - \frac{10^\texttt{imdb_norm}}{10} \right)$$
...
In case of the 3rd line, the 1 unit increase in `log_votes = log10(imdb_num_votes)` actually corresponds to 10 times increase in `imdb_num_votes`, resulting in the *elastic* multiplicative change $10^{\beta}$ of `imdb_rating`, all else being fixed. The final truncated term $(\texttt{log_votes}-11.3)_+ = I_{\texttt{imdb_num_votes}> 10^{11.3}}^{\scriptsize\text{Yes}} \times \log_{10}(\texttt{imdb_num_votes}/10^{11.3})$ is effectively changes both the average value (intercept) and coefficient of change (slope) for very large number of votes.
...
filter(movies, runtime > 60)[design$imdb_norm - pred > 0.17 & design$imdb_norm > 0.5, ] %>% 
  ...
  knitr::kable() %>% 
  kableExtra::kable_styling("striped") %>% kableExtra::scroll_box(width = "100%", height = "400px")
```
